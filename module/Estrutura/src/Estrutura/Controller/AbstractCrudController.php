...
abstract class AbstractCrudController extends AbstractEstruturaController {
    ...
    public function cadastro($service, $form, $atributos = [], $metodoBuscar = "buscar") {
        $id = Cript::dec($this->params('id'));
        $post = $this->getPost();

        if ($id) {
            $busca = $service->$metodoBuscar($id);
            /**
    ...
    public function gravarEmTabelaNaoIdentity($service, $form) {
        try {
            ...
            /**
             * @var $service AbstractEstruturaService
             */
            $classeServico = get_class($service);
            $serviceBuscar = new $classeServico;
            if ($serviceBuscar->buscar($this->getRequest()->getPost()->get("id"))) {
                return $service->salvar();
            }
